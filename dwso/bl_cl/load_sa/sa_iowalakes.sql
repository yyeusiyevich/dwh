-- DROP PROCEDURE IF EXISTS bl_cl.sa_iowalakes_load();
CREATE OR REPLACE PROCEDURE bl_cl.sa_iowalakes_load()

LANGUAGE plpgsql 
AS $$

DECLARE
  schema_name   TEXT := 'sa_iowalakes_sales';
  table_name    TEXT := 'src_iowalakes_sales';
...
SELECT invoice_and_item_number,
	   ...
	   tax_indicator_id,
	   CURRENT_TIMESTAMP
FROM sa_iowalakes_sales.ext_iowalakes_sales AS ext
WHERE NOT EXISTS (SELECT 1
                  FROM  sa_iowalakes_sales.src_iowalakes_sales AS src
